...
const Header = () => {
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-light header-nav pt-2.5">
      <div className="container custom-container">
        {/* Logo */}
        <div className="logo-wrap">
          <a className="navbar-brand" href="/feed">
            <img src="/assets/images/logo.svg" alt="Buddy Script" className="nav-logo" />
          </a>
        </div>
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          {/* Search Form */}
          <div className="header-form ms-auto">
            <form className="header-form-group">
              <svg className="header-form-svg" xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="none" viewBox="0 0 17 17">
          ...
          {/* Navigation Icons */}
          <ul className="navbar-nav mb-2 mb-lg-0 header-nav-list ms-auto mr-8">
            <NavItem href="/feed" icon="home" active={true} />
            <NavItem href="/friend-request" icon="friends" />
            <NotificationItem />
            <NavItem href="/chat" icon="chat" count={2} />
          </ul>
...
const NavItem = ({ href, icon, active = false, count = 0 }) => {
  const getIcon = () => {
    switch (icon) {
      case 'home':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="21" fill="none" viewBox="0 0 18 21">
            <path className={active ? "home-active" : ""} stroke="#000" strokeWidth="1.5" strokeOpacity=".6" d="M1 9.924c0-1.552 0-2.328.314-3.01.313-.682.902-1.187 2.08-2.196l1.143-.98C6.667 1.913 7.732 1 9 1c1.268 0 2.333.913 4.463 2.738l1.142.98c1.179 1.01 1.768 1.514 2.081 2.196.314.682.314 1.458.314 3.01v4.846c0 2.155 0 3.233-.67 3.902-.669.67-1.746.67-3.901.67H5.57c-2.155 0-3.232 0-3.902-.67C1 18.002 1 16.925 1 14.77V9.924z" />
            <path className={active ? "home-active" : ""} stroke="#000" strokeOpacity=".6" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M11.857 19.341v-5.857a1 1 0 00-1-1H7.143a1 1 0 00-1 1v5.857" />
          </svg>
        );
      case 'friends':
        return (
  ...
  return (
    <li className="nav-item header-nav-item">
      <a className={`nav-link header-nav-link ${active ? 'header-nav-link-active' : ''}`} href={href}>
        {getIcon()}
        {count > 0 && <span className="counting">{count}</span>}
      </a>
    </li>
  );
};
...
const NotificationItem = () => {
  ...
  return (
    <li className="nav-item header-nav-item">
      <span 
        className="nav-link header-nav-link header-notify-btn cursor-pointer"
      ...
      >
        ...
        <span className="counting">6</span>
